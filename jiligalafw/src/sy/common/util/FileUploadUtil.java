...
public class FileUploadUtil {
	...
	private static final String deployPath = bundle.getString("server.deployPath");
	private static final String uploadFolder = bundle.getString("server.uploadFolder");
	private static final String maxUploadSize = bundle.getString("spring.maxUploadSize");

	private final static int BUFFER = 1024;
	private final static List<String> allowedFileExtensions = new ArrayList<String>();
	static{
	...
	public static HashMap<String, Object> uploadFileWithLimit(MultipartHttpServletRequest multipartRequest, String nameParam) {
		...
		try {
			...
			else if (file.getSize() > Long.valueOf(maxUploadSize)) {
				isSuccess = false;
				errMsg = "上传失败，最大允许上传大小为 30M";
			}
			
			// 检测扩展名是否合法
			else if (!isAllowedExtensions(fileExt)) {
				isSuccess = false;
				errMsg = "无效的文件类型<BR>允许范围："+ allowedFileExtensions.toString();;
				
			}
			
			// 开始上传文件
			if (isSuccess) {
				String newFileName = DateUtil.getTodayDateTimeSSS() + "." + fileExt;
				
				fileUrl = uploadFolder + "/" + fileExt + "/" + newFileName;
				
				File newFile = new File(deployPath, fileUrl);
				newFile.mkdirs();
	 ...
	 * @return
	 */
	private static boolean isAllowedExtensions(String fileType) {
		if (allowedFileExtensions.contains(fileType.toLowerCase())) {
			return true;
		} else {
			return false;
		}
