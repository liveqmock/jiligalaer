...
<script type="text/javascript">
	function next() {
		var mdmCPUSolns =  document.getElementsByName("mdmCPUSoln");
		var mdmCPUSolnId = "";
		var tag = false;
		for ( var i = 0; i < mdmCPUSolns.length; i++) {
			if (mdmCPUSolns.item(i).checked) {
				tag = true;
				mdmCPUSolnId = mdmCPUSolnId + mdmCPUSolns.item(i).getAttribute("value");
				continue;
			}
		}
		if(mdmCPUSolnId == ''){
		...
		form.action = "${ctx}/combinProd/diskResource.do?mdmCPUSolnId="+mdmCPUSolnId;
		form.submit();
	}
	function previous() {
		var form = document.forms.listForm;
...
<body onLoad="">
	<form:form modelAttribute="baseVoList"
		action="${ctx}/combinProd/computResource.do" method="post"
		name="listForm">
										...
										<td colspan="2">
											<div id="middle_sheet_pm_s_name">
												<img src="${ctx}/images/menu_s_11.jpg" border="0"
													...
													align="absmiddle" id="" /> <img
													src="${ctx}/images/menu_s_21_s.png" border="0"
													align="absmiddle" id="" /> <img
																						...
																						<c:when test="${item.selectFlag==1}">
																							<tr bgcolor="<%=bgcolor %>" height="20">
																								...
																								<td width="10%" align="center" class="f12_1"><input
																									type="radio" value="${item.cellId}__${item.prodId}__${item.priceId}" name="mdmCPUSoln" checked="checked"/></td>
																							</tr>
																						</c:when>
																						<c:otherwise>
																							<tr bgcolor="<%=bgcolor %>" height="20">
																								...
																								<td width="50%" align="left" class="f12_1">${item.displayText}</td>
																								<td width="10%" align="center" class="f12_1"><input
																									type="radio" value="${item.cellId}__${item.prodId}__${item.priceId}" name="mdmCPUSoln" /></td>
																							</tr>
																						</c:otherwise>
																					</c:choose>
																				</c:forEach>
