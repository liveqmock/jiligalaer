...
<script type="text/javascript">
	function next() {
		var mdmDiskSolns = document.getElementsByName("mdmDiskSoln");
		var mdmDiskSolnId = "";
		var tag = false;
		for ( var i = 0; i < mdmDiskSolns.length; i++) {
			if (mdmDiskSolns.item(i).checked) {
				tag = true;
				mdmDiskSolnId = mdmDiskSolnId
						+ mdmDiskSolns.item(i).getAttribute("value");
				continue;
			}
		}
		if(mdmDiskSolnId == ''){
		...
		form.action = "${ctx}/combinProd/infoConfirm.do?mdmDiskSolnId="
				+ mdmDiskSolnId;
		form.submit();
	}
	function previous() {
		var form = document.forms.listForm;
		form.action = "${ctx}/combinProd/computResource.do";
		form.submit();
	}
</script>
...
<body onLoad="">
	<form:form modelAttribute="baseVoList"
		action="${ctx}/combinProd/diskResource.do" method="post"
		name="listForm">
										...
										<td colspan="2">
											<div id="middle_sheet_pm_s_name">
												<img src="${ctx}/images/menu_s_11.jpg" border="0"
													...
													align="absmiddle" id="" /> <img
													src="${ctx}/images/menu_s_22_s.png" border="0"
													align="absmiddle" id="" /> <!--<img
																	...
																	<td>
																		...
																		<table width="763" border="0" cellspacing="1"
																			...
																			<tr bgcolor="#92b8c1" height="20">
																				<td width="40%" align="center" class="f12">名称</td>
																		...
																		<%int i=0; %>
																		<div
																			style="height: 350px; z-index: 1; overflow-y: scroll; overflow-x: hidden;">
																			<table width="763" border="0" cellspacing="1"
																				cellpadding="1" bgcolor="#eeeeee">
																				<c:forEach var="item" items="${cloudMdmDiskSolnVoList}">
																				<%
																					...
																					<c:choose>
																						<c:when test="${item.selectFlag==1}">
																							<tr bgcolor="<%=bgcolor %>" height="20">
																								<td width="40%" align="left" class="f12_1">${item.prodName}</td>
																								<td width="50%" align="left" class="f12_1">${item.diskSize}${item.displayText}</td>
																								<td width="10%" align="center" class="f12_1"><input
																									type="radio" value="${item.cellId}__${item.prodId}__${item.priceId}" name="mdmDiskSoln" checked="checked"/></td>
																							</tr>
																						</c:when>
																						<c:otherwise>
																							<tr bgcolor="<%=bgcolor %>" height="20">
																								...
																								<td width="50%" align="left" class="f12_1">${item.diskSize}${item.displayText}</td>
																								<td width="10%" align="center" class="f12_1"><input
																									type="radio" value="${item.cellId}__${item.prodId}__${item.priceId}" name="mdmDiskSoln" /></td>
																							</tr>
																						</c:otherwise>
																					</c:choose>
																				</c:forEach>
																					<tr bgcolor="eef6f7" height="20">
																						<td width="40%" align="left" class="f12_1">No, Thanks.</td>
																						<td width="50%" align="left" class="f12_1">No, Thanks.</td>
																						<td width="10%" align="center" class="f12_1">
																							<input type="radio" value="____" name="mdmDiskSoln" />
																						</td>
																					</tr>
																			</table>
																		</div>
