...
<script type="text/javascript">
	function next() {
		var mdmTemplates =  document.getElementsByName("mdmTemplate");
		var mdmTemplateId = "";
		var tag = false;
		for ( var i = 0; i < mdmTemplates.length; i++) {
			if (mdmTemplates.item(i).checked) {
				tag = true;
				mdmTemplateId = mdmTemplateId + mdmTemplates.item(i).getAttribute("value");
				continue;
			}
		}
		if(mdmTemplateId == ''){
		...
		form.action = "${ctx}/combinProd/computResource.do?mdmTemplateId="+mdmTemplateId;
		form.submit();
	}
	function previous() {
		var form = document.forms.listForm;
...
<body onLoad="">
	<form:form modelAttribute="baseVoList"
		action="${ctx}/combinProd/selectTemplate.do" method="post"
		name="listForm">
										...
										<td colspan="2">
											<div id="middle_sheet_pm_s_name">
												<img src="${ctx}/images/menu_s_11.jpg" border="0"
													...
													src="${ctx}/images/menu_s_6_s.png" border="0"
													align="absmiddle" id="" /> <img
																	...
																	<td>
																		...
																		<table width="763" border="0" cellspacing="1"
																			...
																			<tr bgcolor="#92b8c1" height="20">
																				<td width="40%" align="center" class="f12">模板名称</td>
																		...
																		<div
																			...
																			<table width="763" border="0" cellspacing="1"
																				cellpadding="1" bgcolor="#eeeeee">
																				<c:forEach var="item" items="${cloudMdmTemplateVoList}">
																					<%
																						...
																						<c:when test="${item.selectFlag==1}">
																							<tr bgcolor="<%=bgcolor %>" height="20">
																								...
																								<td width="10%" align="center" class="f12_1"><input
																									type="radio" value="${item.cellId}__${item.prodId}__${item.priceId}" name="mdmTemplate" checked="checked"/></td>
																							</tr>
																						</c:when>
																						<c:otherwise>
																							<tr bgcolor="<%=bgcolor %>" height="20">
																								...
																								<td width="50%" align="left" class="f12_1">${item.displayText}</td>
																								<td width="10%" align="center" class="f12_1"><input
																									type="radio" value="${item.cellId}__${item.prodId}__${item.priceId}" name="mdmTemplate"/></td>
																							</tr>
																						</c:otherwise>
																					</c:choose>
