...
public class Constants {
	...
	/** 用户审批邮件通知主题 */
	public static final String USER_APPROVAL_MAIL_THEME = BaseConfigurator.getInstance().getCompanyName() + "@userType审批邮件";
	/** 用户审批拒绝邮件模板 */
	public static final String USER_APPROVAL_REJECT_TEMPLATE = "，您好！<br>@userType{userAccount}因为以下原因未通过审批：";
	/** 用户审批通过邮件模板 */
	public static final String USER_APPROVAL_PASS_TEMPLATE = "，您好！<br>@userType{userAccount}已通过审批！<br>"
		+ BaseConfigurator.getInstance().getWelcomeContent()+"<br>系统登录地址：" + BaseConfigurator.getInstance().getServerAddress();
